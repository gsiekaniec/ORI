...
def iterFastq (file: str, threshold: int, length:int, gz: bool, nb_read_tot: int, general_length: int) -> (tuple):
    '''Prend un fichier fastq (ouverture en binaire)'''
    if gz:
        f = gzip.open(file, 'rb')
    else :
        f = open(file,'rb')
    ...
    for header in faiter:
        nb_read_tot += 1
        headerStr = str(header.__next__(), 'utf-8').strip()
        seqQual = "".join(str(s, 'utf-8').strip() for s in faiter.__next__())
        seqQual = seqQual.split('+') 
        qual = []
        general_length += len(seqQual[0])
        if len(seqQual[0]) > length:
            for char in '+'.join(seqQual[1:]):
                q = ord(char)-33
                error_rate = 10**(-(q/10))
                qual.append(error_rate)
            read_quality = -10*(math.log10(statistics.mean(qual)))
            if read_quality >= threshold :
                yield (headerStr, seqQual[0], '+'.join(seqQual[1:]))
