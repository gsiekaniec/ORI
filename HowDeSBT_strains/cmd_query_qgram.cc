...
void QueryQCommand::short_description(std::ostream &s)
{
  s << commandName << "-- query a sequence bloom tree with qgrams" << endl;
}

void QueryQCommand::usage(std::ostream &s,
                          const string &message)
{
  ...
  s << "                       query file" << endl;
  s << "  --threshold=<F>      fraction of query qgrams that must be present in a leaf" << endl;
  ...
  s << "  --adjust             adjust reported number of qgrams present, compensating" << endl;
  s << "                       for bloom filter false positives" << endl;
  s << "  --sort               sort matched leaves by the number of query qgrams present," << endl;
  s << "                       and report the number of qgrams present" << endl;
  ...
  s << "  --distinctkmers      perform the query counting each distinct qgram only once" << endl;
  s << "                       (by default we count a query qgram each time it occurs)" << endl;
  ...
  s << "  --justcountkmers     just report the number of qgrams in each query, and quit" << endl;
  s << "  --countallkmerhits   report the number of qgrams that 'hit', for each" << endl;
  s << "                       query/leaf" << endl;
...
void QueryQCommand::debug_help(std::ostream &s)
{
...
void QueryQCommand::parse(int _argc,
                          char **_argv)
{
  int argc;
  char **argv;
...
QueryQCommand::~QueryQCommand()
{
  for (const auto &q : queries)
    delete q;
}

void QueryQCommand::read_queries_qgram()
{
  if (queryFilenames.empty())
  {
    int aaa = 1;
    //Query::read_query_file_qgram(cin, generalQueryThreshold, queries);
  }
  else
  {
    int numQueryFiles = queryFilenames.size();
    for (int queryIx = 0; queryIx < numQueryFiles; queryIx++)
    {
      ...
      Query::read_query_file_qgram(filename, queryThresholds[queryIx], queries);
      in.close();
    }
  }
}

int QueryQCommand::execute()
{
  ...
  if (justReportKmerCounts)
  {
    ...
    for (auto &q : queries)
    {
      q->qgramize(bf, seeds, distinctKmers, true);
  ...
  else if (countAllKmerHits)
  {
    ...
    //root->batch_count_kmer_hits(queries, onlyLeaves, distinctKmers); //TODO write wrapper around batch query to allow qgram support
    root->batch_count_qgram_hits(queries, seeds, onlyLeaves, distinctKmers);
    // report results
  ...
  else
  {
    ...
    root->batch_query_qgram(queries, seeds, onlyLeaves, distinctKmers, completeKmerCounts, adjustKmerCounts);
...
void QueryQCommand::sort_matches_by_kmer_counts(void)
{
  if (adjustKmerCounts)
  {
    for (auto &q : queries)
...
void QueryQCommand::print_matches(std::ostream &out) const
{
  for (auto &q : queries)
  {
...
void QueryQCommand::print_matches_with_kmer_counts(std::ostream &out) const
{
...
void QueryQCommand::print_kmer_hit_counts(std::ostream &out) const
{
  std::ios::fmtflags saveOutFlags(out.flags());
